...
async function extractSnippets(postIdOrSlug, options = {}) {
  const {
    ...
    apiKey = process.env.GHOST_ADMIN_KEY,
  ...
  if (!apiUrl || !apiKey) {
    throw new Error('GHOST_API_URL and GHOST_ADMIN_KEY environment variables required');
  }
  ...
  // Import ghost-api functions
  const ghostApiPath = join(__dirname, '..', 'scripts', 'ghost-api.js');
  const { ghostApi } = await import(ghostApiPath);

  console.log(`📖 Fetching post: ${postIdOrSlug}`);
  
  // Fetch post with lexical format
  const response = await ghostApi(`/posts/${postIdOrSlug}/?formats=lexical`, 'GET');
